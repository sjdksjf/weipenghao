...
Page({
  ...
  onLoad: function (options) {
    ...
    console.log(new Date().getTime().toString().slice(0, 10) < options.start_time)
    if (new Date().getTime().toString().slice(0, 10)<options.start_time){
      console.log(12345)
      this.setData({
        the_now_name:false
      })
    ...
    var the_time_detail = 'start_time=' + options.start_time + '&end_time=' + options.end_time + '&p=1'
  ...
  changenumadd: function () {
    if (this.data.goodscommitnum+ 1> this.data.the_rob_goods.buy_limit){
      wx.showToast({
        title: '超过该物品购买上限',
        image:'./images/sleep.png'
      })
